...
public class LearnScores {
    ...
    Bot[] nextGeneration(Bot[] oldGeneration) throws InterruptedException {
        ...
        for (int i = 0; i < 4; i++) {
            ...
            for (int j = 0; j < 2; j++) {
                ...
                reversi.initialize(new DefaultReferee(reversi), new ReversiAIMiniMax(reversi), new ReversiAIMiniMax(reversi));
                int finalJ = j;
        ...
        System.out.println("All games done");
    ...
    double[][] mutateScores(double[][] old) {
        ...
        for (int i = 0; i < 4; i++) {
            ...
            // gradient descent:
            a[j] += (Math.random() - .5) * Math.min(.001f, (1000f - generationNr) / 1000f);
        }
